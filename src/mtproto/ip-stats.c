...
struct ip_ht_slot {
  ...
  int ready;          /* geo fields published after first writer initializes */
...
static struct ip_ht_slot *ip_ht_wait_ready (struct ip_ht_slot *s) {
  struct timespec start, now;
  clock_gettime (CLOCK_MONOTONIC, &start);
  while (!__atomic_load_n (&s->ready, __ATOMIC_ACQUIRE)) {
    sched_yield ();
    clock_gettime (CLOCK_MONOTONIC, &now);
    if (now.tv_sec - start.tv_sec >= 2) return NULL;
  }
  return s;
}
...
static struct ip_ht_slot *ip_ht_find (uint32_t ip) {
  ...
  for (int scanned = 0; scanned < IP_HT_SIZE; scanned++) {
    struct ip_ht_slot *s = &shared->ht[idx];
    uint32_t cur = __atomic_load_n (&s->ip, __ATOMIC_ACQUIRE);
    if (cur == ip) {
      return ip_ht_wait_ready (s);
    }
...
static struct ip_ht_slot *ip_ht_get_or_create (uint32_t ip, int *is_new_ip) {
  ...
  __sync_fetch_and_add (&shared->table_full_total, 1);
  return NULL;
}
...
static struct country_stats_entry *country_find_or_create (const char *code) {
  ...
  int count = __atomic_load_n (&cs->count, __ATOMIC_ACQUIRE);
  for (int i = 0; i < count; i++) {
    if (cs->entries[i].code[0] == code[0] && cs->entries[i].code[1] == code[1]) {
      return &cs->entries[i];
    }
  }
  while (__sync_lock_test_and_set (&cs->lock, 1)) sched_yield ();
  count = cs->count;
  for (int i = 0; i < count; i++) {
    if (cs->entries[i].code[0] == code[0] && cs->entries[i].code[1] == code[1]) {
      __sync_lock_release (&cs->lock);
      return &cs->entries[i];
    }
  }
  if (count >= 256) {
  ...
  struct country_stats_entry *e = &cs->entries[count];
  e->code[0] = code[0];
  e->code[1] = code[1];
  e->code[2] = 0;
  __atomic_store_n (&cs->count, count + 1, __ATOMIC_RELEASE);
  __sync_lock_release (&cs->lock);
  return e;
}
...
static struct region_stats_entry *region_find_or_create (const char *code) {
  ...
  int count = __atomic_load_n (&rs->count, __ATOMIC_ACQUIRE);
  for (int i = 0; i < count; i++) {
    if (strcmp (rs->entries[i].code, code) == 0) {
      return &rs->entries[i];
    }
  }
  while (__sync_lock_test_and_set (&rs->lock, 1)) sched_yield ();
  count = rs->count;
  for (int i = 0; i < count; i++) {
    if (strcmp (rs->entries[i].code, code) == 0) {
      __sync_lock_release (&rs->lock);
      return &rs->entries[i];
    }
  }
  ...
  struct region_stats_entry *e = &rs->entries[count];
  strncpy (e->code, code, 7);
  e->code[7] = 0;
  __atomic_store_n (&rs->count, count + 1, __ATOMIC_RELEASE);
  __sync_lock_release (&rs->lock);
  return e;
}
...
void ip_stats_transport_seen (uint32_t ip, int transport) {
  ...
  } while (!__atomic_compare_exchange_n (&s->transport_seen_mask, &old_mask,
                                         old_mask | mask, 0,
...
void ip_stats_prometheus (stats_buffer_t *sb, int top_n) {
  ...
  sb_printf (sb,
    "# HELP teleproxy_ip_stats_table_full_total IP tracking attempts dropped because the table was full.\n"
    "# TYPE teleproxy_ip_stats_table_full_total counter\n"
    "teleproxy_ip_stats_table_full_total %lld\n", shared->table_full_total);
