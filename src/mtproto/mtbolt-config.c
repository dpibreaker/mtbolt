...
static void cfg_read_int_marked (toml_datum_t tab, const char *key, int *dst, int *present) {
  toml_datum_t v = toml_get (tab, key);
  if (v.type == TOML_INT64 && v.u.int64 >= INT32_MIN && v.u.int64 <= INT32_MAX) {
    *dst = (int) v.u.int64;
    *present = 1;
  }
}
...
static void cfg_read_bool_marked (toml_datum_t tab, const char *key, int *dst, int *present) {
  toml_datum_t v = toml_get (tab, key);
  if (v.type == TOML_BOOLEAN) {
    *dst = v.u.boolean ? 1 : 0;
    *present = 1;
...
int mtbolt_config_load_toml (struct mtbolt_config *cfg, const char *path,
  ...
  if (sec.type == TOML_TABLE) {
    cfg_read_int_marked (sec, "max_connections", &cfg->max_connections, &cfg->max_connections_set);
    cfg_read_int_marked (sec, "backlog", &cfg->backlog, &cfg->backlog_set);
  ...
  if (sec.type == TOML_TABLE) {
    cfg_read_int_marked (sec, "count", &cfg->workers, &cfg->workers_set);
  }

  /* [stats] */
  sec = toml_get (res.toptab, "stats");
  if (sec.type == TOML_TABLE) {
    cfg_read_bool_marked (sec, "http_enabled", &cfg->stats_http_enabled, &cfg->stats_http_enabled_set);
  }
...
#define FAIL(...) do { snprintf (errbuf, errlen, __VA_ARGS__); return -1; } while (0)
  ...
  if (cfg->max_connection_buffer < 1024)
    FAIL ("max_connection_buffer must be >= 1024 (got %d)", cfg->max_connection_buffer);
  if (cfg->workers < 0)
